...
M3_DIR = ROOT / "milestone2" / "continuous_data_of_one_subject"
...
def plot_continuous(recording: Recording, epochs: mne.Epochs) -> None:
    # Re-read the raw for the continuous plot (epochs._raw is None after preload).
    raw_inst = mne.io.read_raw_brainvision(recording.vhdr_path, preload=True, verbose="ERROR")
    rename_brainvision_channels(raw_inst)
    raw_inst.pick("eeg")
    raw_inst.filter(l_freq=0.1, h_freq=30.0, method="iir", iir_params={"order": 4, "ftype": "butter"}, verbose="ERROR")
    raw_inst.set_eeg_reference("average", projection=False, verbose="ERROR")
    bad_channels = detect_bad_channels(raw_inst)
    if bad_channels:
        raw_inst.info["bads"] = bad_channels
        raw_inst.interpolate_bads(reset_bads=True, verbose="ERROR")
    raw_inst = raw_inst.crop(tmin=0.0, tmax=min(30.0, raw_inst.times[-1]))
